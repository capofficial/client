...
<script>
	...
	async function fetchData() {
		items = await getTransactions('type=pool-payin,pool-payout,pool-deposit,pool-withdrawal,fee');
		isLoading = false;
	}
	$: fetchData();
...
<style>
	...
	.table {
		--grid-template: 140px 1fr 160px 1fr 1fr 1fr;
	}
	...
	<div class='table'>
		...
		<div class='table-body'>
			...
			<div class='row'>
				...
				<div class='cell la'>{item.type}</div>
				<div class='cell'>{numberWithCommas(item.amount || item.poolFee)}</div>
				<div class='cell'>{numberWithCommas(item.poolBalance)}</div>
				<div class='cell'>{numberWithCommas(item.bufferBalance)}</div>
			</div>
			{/each}
		</div>
	</div>
